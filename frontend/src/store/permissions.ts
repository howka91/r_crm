/**
 * Permission store (Pinia setup-syntax).
 *
 ...
 *
 * Also syncs CASL so `v-if="$can('access', 'clients.view')"` works in templates.
 */
...
export const usePermissionStore = defineStore("permissions", () => {
  ...
  function setFromStaff(
    staff: { is_superuser: boolean; role: RoleBrief | null } | null,
  ): void {
    if (!staff) {
